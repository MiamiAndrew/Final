pipeline {
    ...
    environment {
        ...
        GITHUB_URL = 'https://github.com/MiamiAndrew/Final.git'     //<-----change this to match this new repository!
        ...
        stage('Deploy to Prod Environment') {
            steps {
                // Set up Kubernetes configuration using the specified KUBECONFIG
                 def kubeConfig = readFile(KUBECONFIG)
                //sh "ls -la"
                sh "sed -i 's|${DOCKER_IMAGE}:latest|${DOCKER_IMAGE}:${GET_TAG}|' deployment-prod.yaml
                sh "cd .."
